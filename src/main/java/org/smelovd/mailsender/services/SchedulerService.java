...
public class SchedulerService {
    ...
    public SchedulerService(@Qualifier("threadPoolTaskScheduler") final TaskScheduler scheduler, final MailService mailService) {
        this.scheduler = scheduler;
        this.mailService = mailService;
    }

    public void scheduleSendingTemplatesToAllUsers(final CronJob cronJob) {
    ...
    public void stopScheduledTaskByCronJobId(final int id) {
        log.info("Stop scheduled task with id: {}", id);
        final var task = scheduledTasks.remove(id);
